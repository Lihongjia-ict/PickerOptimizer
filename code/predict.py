...
class CreateDatasetFromImages(Dataset):
    def __init__(self, file_path, transform_):
        """
        Args:
        ...
        self.image_arr=os.listdir(file_path)
        #print(self.image_arr)
        self.image_arr.sort(key=lambda x:int(x[-7:-4]))
...
def add_arguments(parser):
    ...
    parser.add_argument('-o', '--output-file',type=str, help='predicts output')
    parser.add_argument('-p', '--model-path',type=str, help='model path')
    #parser.add_argument('-i', '--val_file',type=str, help='path of csv file')
...
def predict_model(dataloaders,model,output):
    ...
    for data,fname in dataloaders:
        ...
        print("fname:{}  label:{}".format(fname,preds))
        output_file.write(fname[0]+' '+str(preds.cpu().numpy()[0])+'\n')

    print("particle num :{} | ice num: {}| carbon num:{}".format(particle_nums,ice_nums,carbon_nums))
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(iterations)):
        ...
        #ValDataset = CreateDatasetFromImages(os.path.join(data_dir,'images'),data_transforms['val'])
        ValDataset = CreateDatasetFromImages(data_dir,data_transforms['val'])
        val_loader = torch.utils.data.DataLoader(
            dataset=ValDataset,
            batch_size=1,
        ...
        #model_ft=torch.load("/data/lihongjia/model/classification_code/model_compare/saved_models/resnet/pre_reload_new/10059/20shot/model_iter2_best.sav")
        model_ft=torch.load(model_path,map_location='cpu')
        '''
        model_ft = models.resnet18(pretrained=True)
        num_ftrs = model_ft.fc.in_features
        num_classes=args.num_classes
        ...
        predict_model(dataloaders=val_loader,model=model_ft,output=output_file)
        

        #model_ft_,best_acc = train_model(dataloaders=val_dataloaders,model=model_ft,criterion=criterion,optimizer=optimizer_ft, scheduler=exp_lr_scheduler,num_epochs=args.num_epochs,save_prefix=args.save_prefix)
